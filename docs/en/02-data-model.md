...
| `document_insights`    | Reviewable document intelligence drafts generated by worker |
...
- `job_type`: transcribe_audio, extract_visit, generate_visit_summary, suggest_issues, suggest_decisions, generate_weekly_summary, analyze_document
...
Phase 12 adds `document_insights` plus the `analyze_document` worker job type. Insights are
reviewable drafts generated only by the worker:

- `agent_jobs.input = { "documentId": "uuid" }`.
- The worker downloads from the private `project-documents` bucket using the service role key.
- Only text-like documents (`text/*`, for example `text/plain` and `text/csv`) are processed.
- Images/photos, PDFs, Office binaries and other non-text files fail permanently; there is no OCR,
  image analysis, document parsing or document intelligence inside web requests.
- Results are stored in `document_insights` with `source = 'ai'`, `review_state = 'ai_draft'`
  and `created_by_job_id` pointing to the job that produced them.
...
- **Document insight provenance** lives on `document_insights`: `created_by_job_id` points to the
  worker job, and human review fields mirror the weekly summary pattern.
